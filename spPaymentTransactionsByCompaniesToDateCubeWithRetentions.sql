...
EXECUTE THIS QUERY FOR DAILY ETL PLAN:
	...
	EXEC [DBO].[spPaymentTransactionsByCompaniesToDateCubeWithRetentions] @DailySPDate
...
USE DWH_Workspace
GO
CREATE PROCEDURE [dbo].[spPaymentTransactionsByCompaniesToDateCubeWithRetentions] (@BaseDay DATE) AS
...
DELETE FROM  DWH_Workspace.[DB\skacar].[FACT_PaymentTransactionsByCompaniesToDateCubeWithRetentions] WHERE [DateHour] >= @DailySP AND [DateHour] < @BaseDay
;
...
--			  ,DATEFROMPARTS(YEAR(CreatedAt),(((MONTH(CreatedAt))-1)/2)*2+1,1) Indicator_2MTD
		...
		from (
			...
			--			,ISNULL(cast(l.[Description] as VARCHAR(120)),'N/A')	[Description]
			--			,UTH.CustomerType
			--			,ROW_NUMBER() OVER (PARTITION BY UTH.CustomerKey ORDER BY UTH.CreatedAt DESC) CustomerTypeHistoryRanker
				...
				FROM (select l.Id,l.CustomerKey,l.Amount,l.CreatedAt, IIF(l.CompanyId IS NULL,-80,L.CompanyId) CompanyId/*, IIF(l.[Description] IS NULL,'NULL',REPLACE(REPLACE(l.[Description],'ödemesi','Payment'),'payment','Payment')) [Description]*/
							,IIF(ECA.PaymentType IS NULL,-80,ECA.PaymentType) PaymentType
					  from [DWH_DB]..[FACT_Transactions] l WITH (Nolock)
					  left join DB_billpayment..DIM_Company eca with (nolock) on eca.Id = l.CompanyId
					  where FeatureType = 17 AND IsCancellation = 0 AND OperatorCustomerKey IS NULL AND YEAR(l.CreatedAt) >= YEAR(DATEADD(YEAR,-@y,@BaseDay)) AND l.CreatedAt < @BaseDay) L
				JOIN [DWH_DB]..[DIM_Customers] u with (nolock)  on u.Customer_Key = l.CustomerKey
			--	JOIN  DWH_DB..DIM_CustomerTypeHistory UTH WITH (NOLOCK) ON UTH.CustomerKey = l.CustomerKey AND l.CreatedAt >= UTH.CreatedAt
			 ) m1
...
--				 ,DATEFROMPARTS(YEAR(CreatedAt),(((MONTH(CreatedAt))-1)/2)*2+1,1)
				 ,cube(PaymentType,CompanyId/*,[Description]*//*,CustomerType*/)
...
(
					Select
						...
						,SUM(SUM(TxCount))			  OVER (PARTITION BY [Date],PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxCountDTH
						,SUM(SUM(ABS(TotalVolume)))	  OVER (PARTITION BY [Date],PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxVolumeDTH
						,SUM(Age)/count(case when Age IS NOT NULL THEN 1 else NULL END) AvgAgeHourly
						,AVG(TenureByYear)												AvgTenureByYearHourly
						,SUM(SUM(Age))													   OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when Age IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgAgeDTH
						,SUM(SUM(Age))													   OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when Age IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgAgeMTD
						,SUM(SUM(Age))													   OVER (PARTITION BY YEAR([Date]),QuarterNumber,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when Age IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),QuarterNumber,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgAgeQTD
						,SUM(SUM(Age))													   OVER (PARTITION BY YEAR([Date]),SemiYearIndicator,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when Age IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),SemiYearIndicator,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgAgeSemiYTD
						,SUM(SUM(Age))													   OVER (PARTITION BY YEAR([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when Age IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgAgeYTD
						,SUM(SUM(TenureByYear))														OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when TenureByYear IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgTenureByYearDTH
						,SUM(SUM(TenureByYear))														OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when TenureByYear IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgTenureByYearMTD
						,SUM(SUM(TenureByYear))														OVER (PARTITION BY YEAR([Date]),QuarterNumber,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when TenureByYear IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),QuarterNumber,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgTenureByYearQTD
						,SUM(SUM(TenureByYear))														OVER (PARTITION BY YEAR([Date]),SemiYearIndicator,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when TenureByYear IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),SemiYearIndicator,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgTenureByYearSemiYTD
						,SUM(SUM(TenureByYear))														OVER (PARTITION BY YEAR([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
								SUM(count(case when TenureByYear IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgTenureByYearYTD
						,SUM(SUM(TxCount))			OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxCountMTD
						,SUM(SUM(ABS(TotalVolume))) OVER (PARTITION BY YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxVolumeMTD
						,SUM(SUM(TxCount))			OVER (PARTITION BY YEAR([Date]),QuarterNumber,				PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxCountQTD
						,SUM(SUM(ABS(TotalVolume))) OVER (PARTITION BY YEAR([Date]),QuarterNumber,				PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxVolumeQTD
						,SUM(SUM(TxCount))			OVER (PARTITION BY YEAR([Date]),SemiYearIndicator,			PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxCountSemiYTD
						,SUM(SUM(ABS(TotalVolume))) OVER (PARTITION BY YEAR([Date]),SemiYearIndicator,			PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxVolumeSemiYTD
						,SUM(SUM(TxCount))			OVER (PARTITION BY YEAR([Date]),							PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxCountYTD
						,SUM(SUM(ABS(TotalVolume))) OVER (PARTITION BY YEAR([Date]),							PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxVolumeYTD
						--,SUM(SUM(Age))													   OVER (PARTITION BY YEAR([Date]),Indicator_2MTD,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
						--		SUM(count(case when Age IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),Indicator_2MTD,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgAge2MTD

						--,SUM(SUM(TenureByYear))														OVER (PARTITION BY YEAR([Date]),Indicator_2MTD,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour)*1.0 / 
						--		SUM(count(case when TenureByYear IS NOT NULL THEN 1 else NULL END)) OVER (PARTITION BY YEAR([Date]),Indicator_2MTD,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) AvgTenureByYear2MTD


						--,SUM(SUM(TxCount))			OVER (PARTITION BY YEAR([Date]),Indicator_2MTD,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxCount2MTD
						--,SUM(SUM(ABS(TotalVolume))) OVER (PARTITION BY YEAR([Date]),Indicator_2MTD,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ ORDER BY ContributedDateHour) TxVolume2MTD

					From CustomerBasedCTE
					WHERE ContributedDateHour < @BaseDay
					Group By [Date],ContributedDateHour,SemiYearIndicator,QuarterNumber/*,Indicator_2MTD*/,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/
					)
 ...
 from
		(
			select	
				...
				/*,[Description]*/
				/*,CustomerType*/
				,RANK() OVER (Partition by YEAR(ContributedDateHour),MONTH(ContributedDateHour),DAY(ContributedDateHour),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ Order By ContributedDateHour,CustomerKey) Ranker
			from
					(
					Select
					...
					Group By CustomerKey,PaymentType,CompanyId/*,[Description]*/
					) M
		) R
 GROUP BY ContributedDateHour,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/
...
(
					...
					FROM
							...
							select
								...
								/*,[Description]*/
								/*,CustomerType*/
								,RANK() OVER (Partition by YEAR([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ Order By ContributedDateHour,CustomerKey) Ranker
							from
									(
									...
									where	[Date] >= @Param_QTDIndicator
										and [Date] <  @BaseDay
									group by CustomerKey,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/
									) z
							  ) T
					GROUP BY ContributedDateHour,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/
)
 ...
 , DailyWithMTDForUU  AS
					...
					FROM
							...
							select
								...
								/*,[Description]*/
								/*,CustomerType*/
								,RANK() OVER (Partition by YEAR([Date]),MONTH([Date]),PaymentType,CompanyId/*,[Description]*//*,CustomerType*/ Order By ContributedDateHour,CustomerKey) Ranker
							from
									(
									select MIN(ContributedDateHour) ContributedDateHour
										  ,MIN([Date]) [Date]
 ...
 , DailyWithYTDForUU  AS
					...
					FROM
							...
							from
									...
									from CustomerBasedCTE
									where [Date] <=  @BaseDay
									group by PaymentType,CustomerKey,CompanyId/*,[Description]*//*,CustomerType*/
									) z
							  ) T
					GROUP BY ContributedDateHour,PaymentType,CompanyId/*,[Description]*//*,CustomerType*/
				    )
...
, CombiningDummyDataWithFundamental AS
				...
				--	INTO DWH_Workspace..[FACT_PaymentTransactionsAndCompanyToDateCube]
					...
					LEFT JOIN DailyWithYTDForUU								   G1 on m1.DateHour = G1.ContributedDateHour  and m1.PaymentType = G1.PaymentType  and m1.CompanyId = G1.CompanyId --and m1.[Description]=m3.[Description]--and m1.CustomerType = m3.CustomerType
					LEFT join DailyWithSemiYTDForUU							   m3 on m1.DateHour = m3.ContributedDateHour  and m1.PaymentType = m3.PaymentType  and m1.CompanyId = m3.CompanyId --and m1.[Description]=m3.[Description]--and m1.CustomerType = m3.CustomerType
					LEFT join DailyWithQTDForUU								   m4 on m1.DateHour = m4.ContributedDateHour  and m1.PaymentType = m4.PaymentType  and m1.CompanyId = m4.CompanyId --and m1.[Description]=m4.[Description]--and m1.CustomerType = m4.CustomerType
					--LEFT join DailyWith2MTDForUU							   a1 on m1.DateHour = a1.ContributedDateHour  and m1.PaymentType = a1.PaymentType  and M1.CompanyId = a1.CompanyId --and m1.[Description]=L1.[Description]--and m1.CustomerType = L1.CustomerType
					LEFT join DailyWithMTDForUU								   m2 on m1.DateHour = m2.ContributedDateHour  and m1.PaymentType = m2.PaymentType  and m1.CompanyId = m2.CompanyId --and m1.[Description]=m2.[Description]--and m1.CustomerType = m2.CustomerType
					LEFT join DailyUniqueCustomerCalculation					   c1 on M1.DateHour = c1.ContributedDateHour  and m1.PaymentType = c1.PaymentType  and m1.CompanyId = c1.CompanyId --and m1.[Description]=c1.[Description]
					LEFT join DailyWithAndWithoutNonDistinctToDateCalculations L1 on m1.DateHour = L1.ContributedDateHour  and m1.PaymentType = L1.PaymentType  and M1.CompanyId = L1.CompanyId --and m1.[Description]=L1.[Description]--and m1.CustomerType = L1.CustomerType
...
--INSERT INTO DWH_Workspace.[DB\skacar].[FACT_PaymentTransactionsByCompaniesToDateCubeWithRetentions]
